...
\\
\\
\math u \times v = det(u,v) = \vert u \vert \vert v \vert sin(\phi)\) 

- Area of parallelogram spanned by the two vectors u and v 

- Can be used with $proj_v$u to determine length of point to line
...
- \math 0 + x = x\) (0 is additive identity)
...
- \math x \times 1 = x\) (1 is multiplicative identity)
...
- \math 1 \times x = x\) (1 is multiplicative identity)
...
- \math gcd(a,b) = c, \exists t,s\) such that \math ta + sb = c\)
...
- If \math (a,b) ∼ (a',b')\) and \matha, a' \neq -0\) then \math (a,b)^-1 ∼ (a',b')^-1\)
\\
\\
Definitions:

- \math cl(a,b) + cl(c,d) = cl((a,b) + (c,d))\)

- \math -cl(a,b) = cl(-a,b)\)

- \math cl(a,b) \times cl(c,d) = cl(ac,bd)\)

- If \math x \neq 0\) then \math x \times x^-1 = x^-1 \times x = 1\) (reciprocal is a multiplicative inverse)
\\
\\
1. An integer is non-negative if it is in the image of the map \math i:N \rightarrow Z\)
\newline 2. A rational number is non-negative if it has a representative (a,b) with a and b being non-negative integers
\newline 3. If x and y are rational, then \math x \leq y\) if \math y - x\) is non-negative (\leq\) is a total order and satisfies monotonicity properties)
\\
\\
\math 1 \neq 0\) (Fields have at least two elements)
\\
\\
Ordered field, a field F together with a total order \leq\) such that additionally if \math x,y,t \in F\) with \math x \leq y\) then:

- \math x + t \leq y + t\) (relationship between \leq\) and +)

- \math t \geq 0\) then \math x \times t \leq y \times t\) (relationship between \leq\) and \times\))
\newline Let X be a totally ordered set

- Subset S of X is called "bounded above" if there is an element B \in\) X such that \forall x \in S, x \leq B\)

- If S is "bounded above", then any B \in\) X such that \forall x \in S, x \leq B\) is called an "upper bound" for S
\newline We say that an element L \in\) X is a "least upper bound" for S if

- L is an "upper bound" for S

- If x \math <\) L then x is not an "upper bound" for S
\newline Completeness: A totally ordered set X is said to be complete if every nonempty bounded-above subset of X has a least upper bound
